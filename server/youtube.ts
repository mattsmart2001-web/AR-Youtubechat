...
export class YouTubeChatPoller {
  ...
  constructor(auth: OAuth2Client, liveChatId: string, onMessage: (msg: ChatMessage) => void) {
  ...
  private scheduleNext(): void {
    ...
    this.timer = setTimeout(async () => { await this.fetch(false); this.scheduleNext(); }, this.pollIntervalMs);
  }
  ...
  private async fetch(seed: boolean): Promise<void> {
    try {
      ...
      this.pollIntervalMs = Math.max(data.pollingIntervalMillis ?? 5000, MIN_POLL_INTERVAL_MS);
      ...
      for (const item of data.items ?? []) {
        ...
        if (text) this.onMessage({ id, author, text, timestamp: new Date().toISOString() });
      }
    } catch (err: unknown) {
      console.error('[YouTube] Poll error:', err instanceof Error ? err.message : String(err));
    }
  }
}
